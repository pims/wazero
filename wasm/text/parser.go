...
func (p *moduleParser) beginField(tok tokenType, fieldName []byte, _, _ uint32) error {
	...
	// We expect p.currentLocalField set according to a potentially nested "($fieldName".
...
func (p *moduleParser) endField() {
	...
	default: // currentLocalField is an enum, we expect to have handled all cases above. panic if we didn't
...
func (p *moduleParser) parseImportFuncEnd(tok tokenType, tokenBytes []byte, _, _ uint32) error {
	if tok == tokenRParen {
		fn := p.module.importFuncs[p.currentFuncIndex]
		var localNames wasm.NameMap
		fn.typeIndex, fn.typeInlined, localNames = p.typeParser.getTypeUse()
		if localNames != nil {
			p.module.paramNames =
				append(p.module.paramNames, &wasm.NameMapAssoc{Index: p.currentFuncIndex, NameMap: localNames})
		}
		p.currentField = fieldModuleImport
		p.tokenParser = p.parseImport
		return nil
...
func (p *moduleParser) parseFuncEnd(tok tokenType, tokenBytes []byte, _, _ uint32) error {
	if tok == tokenRParen {
		...
		var paramNames wasm.NameMap
		fn.typeIndex, fn.typeInlined, paramNames = p.typeParser.getTypeUse()
		if paramNames != nil {
			p.module.paramNames =
				append(p.module.paramNames, &wasm.NameMapAssoc{Index: p.currentFuncIndex, NameMap: paramNames})
		}
		fn.code = p.funcParser.getCode()
